...
export class AppMainComponent implements OnInit,OnDestroy {
  ...
  public appSummaryMemoryMaxFixed: string;
  public appSummaryMemory: number;
  public appSummaryDiskMax: number;
  public appSummaryDiskMaxFixed: string;
    ...
    this.appMainService.getCodeMax('APP_DISK_SIZE').subscribe(data => {
      data.list.some(r => {
        if (r.key === 'max_size') {
          ...
          this.appSummaryDiskMaxFixed = this.appSummaryDiskMax.toFixed(1);
          return true;
        }
        ...
        codedata.list.some(r => {
          if (r.key === 'max_size') {
            if (r.value > data.quota.memoryLimit) {
              ...
              this.appSummaryMemoryMaxFixed = this.appSummaryMemoryMax.toFixed(1);
            } else {
          ...
          this.appSummaryMemoryMaxFixed = this.appSummaryMemoryMax.toFixed(0);
        });
      });
    ...
    this.appMainService.updateAlarm(params).subscribe(data => {
      if (data) {
        if (data.status == "success") {
          this.common.alertMessage(this.translateEntities.alertLayer.scalingSucceed, true);
          this.ngOnInit();
          $("[id^='layerpop']").modal("hide");
        } else {
          $("[id^='layerpop']").modal("hide");
  ...
  addAppRoute() {
    ...
    this.appMainService.addAppRoute(params).subscribe(data => {
      ...
      /*reset*/
      $("#routeAddHostName").val("");
      $("#routeAddPathName").val("");
    });
  }
  ...
  getCpuChart() {
    ...
    this.appMainService.getCpuUsage(guid, idx, defaultTimeRange, groupBy, type).subscribe(data => {
      ...
      this.cpu_Chart = new Chart(speedCanvas, {
        type: 'line',
        data: speedData,
  ...
  getMemoryChart() {
    ...
    this.appMainService.getMemoryUsage(guid, idx, defaultTimeRange, groupBy, type).subscribe(data => {
      ...
      this.memory_Chart = new Chart(speedCanvas2, {
        type: 'line',
        data: speedData,
        options: chartOptions
      });
  ...
  getNetworkByte() {
    ...
    this.appMainService.getNetworkByte(guid, idx, defaultTimeRange, groupBy, type).subscribe(data => {
      ...
      this.diskIO_Chart = new Chart(speedCanvas3, {
        type: 'line',
        data: speedData,
        options: chartOptions
      });
