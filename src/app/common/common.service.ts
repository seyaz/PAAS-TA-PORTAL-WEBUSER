...
export class CommonService {
  ...
  doGetCaas(sub_url: string){
    var _headers = new HttpHeaders();
    return this.http.get(this.getCaaSApiUri()+sub_url, {
      headers: _headers.set('Authorization', this.getCaaSAuthorization())
        .set("Content-Type","application/json")
    });
  }
  ...
  doGetExternal(url: string, token: string) {
    if (token == null) {
      token = '';
    }
    return this.http.get(url, {
  ...
  doFilePostExternal(url: string, body: any, token: string) {
    if (token == null) {
      token = '';
    }
    this.fileheaders = new HttpHeaders();
  ...
  doPutExternal(url: string, body: any, token: string) {
    if (token == null) {
      token = '';
    }
    return this.http.put(url, body, {
