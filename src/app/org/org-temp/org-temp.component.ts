...
export class OrgTempComponent implements OnInit, AfterViewChecked {
  ...
  getUserSpaceRoles(item: any, spaceid: string, spacename: string) {
    ...
    this.orgMainService.getUserSpaceRoles(this.sltSpaceGuid).subscribe(data => {
      ...
      this.sltEntity.userRoles.user_roles.forEach(function (role, index) {
        $("#modal10_" + index + "_" + spaceid).prop('checked', false);
        $("#modal11_" + index + "_" + spaceid).prop('checked', false);
        $("#modal12_" + index + "_" + spaceid).prop('checked', false);
        ...
        spaceRole.some(model => {
          if (model.entity.username === username) {
            model.entity.space_roles.filter(space_role => {
              ...
              switch (space_role) {
                case 'space_manager' :
                  $("#modal10_" + index + "_" + spaceid).prop('checked', true);
                  break;
                case 'space_developer' :
                  $("#modal11_" + index + "_" + spaceid).prop('checked', true);
                  break;
                case 'space_auditor' :
                  $("#modal12_" + index + "_" + spaceid).prop('checked', true);
                  break;
              }
            });
            return true;
      ...
      //setTimeout(() => this.allCheck3(spaceid), 1000);
      this.allCheck3(spaceid);
      $("#layerpop_SpaceRole_" + spaceid).modal("show");
    }, error => {
    }, () => {
      this.common.isLoading = false;
    });
      ...
      data.result.userRoles.user_roles.forEach(function (role, index) {
        if (role.user_email != null) {
          if (role.roles.indexOf('SpaceManager') > -1) {
            $("#role_" + role.user_email + "_" + spaceid).append("<label name='space_" + orgname + "'>SpaceManager(공간 관리자)&nbsp;&nbsp;&nbsp;&nbsp;</label>");
          }
          if (role.roles.indexOf('SpaceDeveloper') > -1) {
            $("#role_" + role.user_email + "_" + spaceid).append("<label name='space_" + orgname +  "'>SpaceDeveloper(공간 개발자)&nbsp;&nbsp;&nbsp;&nbsp;</label>");
          }
          if (role.roles.indexOf('SpaceAuditor') > -1) {
            $("#role_" + role.user_email +"_" + spaceid).append("<label name='space_" + orgname +  "'>SpaceAuditor(공간 감사자)&nbsp;&nbsp;&nbsp;&nbsp;</label>");
          }
        }
      });
    }, error => {
  ...
  allCheck3(guid: string) {
    $(".checkAll5_" + guid).change(function () {
      $(".checkSel5_" + guid).prop('checked', $(this).prop("checked"));
    });

    $(".checkSel5_" + guid).change(function () {
      var allcount = $(".checkSel5_" + guid).length;
      var ckcount = $(".checkSel5_" + guid + ":checked").length;
      $(".checkAll5_" + guid).prop('checked', false);
      console.log(allcount + "::::" + ckcount);
      if (allcount == ckcount) {
        $(".checkAll5_" + guid).prop('checked', true);
      }
    });
  ...
  createSpace() {
    ...
    let params = {
      ...
      spaceName: $("#createSpaceName_" + this.orgsDetailGuid).val()
    };
    console.log(params);
